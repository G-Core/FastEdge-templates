/**
 * Shared gate-behaviour acceptance suite for the merged cdn-filter template.
 *
 * All three variants (gate-only, cookie, header) run through the same wasm
 * binary now, selected at runtime via SSO_VARIANT. This module lets
 * filter.test.ts exercise the shared gate logic once per variant/tier
...
function variantEnv(variant: string): string {
  return `FASTEDGE_VAR_ENV_SSO_VARIANT=${variant}`;
}
...
/**
 * A signing tier describes how a variant's filter verifies tokens: the SSO_VARIANT
 * this tier represents, the .env lines that supply its verification key, and a
 * factory that mints a token the filter will accept. Gate behaviour (cookie
 * naming, redirect formatting, /auth bypass, expired/tampered/malformed) is
 * alg-agnostic, so the same gate suite runs against any tier — HS256 for
 * gate-only/header, ES256 for cookie.
 */
...
/** HS256 tier — gate-only and header variants verify HS256 tokens. */
export function hs256Tier(variant: "gate-only" | "header" = "gate-only"): GateSigningTier {
  return {
    variant,
    keyEnv: [
...
export async function runGateSuites(
  ...
  report(`Suite A (${tier.variant}) — runtime config (LOGIN_PAGE_URL + SESSION_COOKIE):`, configured);
  ...
  report(`Suite B (${tier.variant}) — defaults:`, defaults);
...
export async function runStripSuites(
  wasmPath: string,
  variant: "gate-only" | "header" = "gate-only",
): Promise<{ totalFailed: number; tempDirs: string[] }> {
  ...
  report(`Suite — cookie stripping (${variant}):`, stripSuite);
...
/**
 * Runs the ES256 signing-tier acceptance suite (cookie variant).
 * Returns { totalFailed, tempDirs } — callers must rm -rf the dirs when done.
 */
export async function runEs256Suites(
  ...
  // Export public key as JWK JSON — stored as SESSION_PUBLIC_JWK in the filter env
...
export async function runAudIssSuites(
  ...
  report(`Suite — aud/iss enforcement (${tier.variant}):`, suite);
...
export async function runCustomPrefixSuites(
  ...
  report(`Suite — custom AUTH_PREFIX (${CUSTOM_PREFIX}, ${tier.variant}):`, suite);
  totalFailed += suite.failed;
...
export async function runFailClosedSuites(
  ...
  report(`Suite — fail-closed audience (${tier.variant}):`, suite);
  totalFailed += suite.failed;
...
/**
 * Fail-closed SSO_VARIANT: with SSO_VARIANT unset or not one of
 * gate-only/cookie/header, the filter can't know which alg to trust or
 * whether to strip/inject anything, so it must refuse every session — even
 * one that would otherwise verify fine under some variant.
 * Returns { totalFailed, tempDirs } — callers must rm -rf the dirs when done.
 */
export async function runVariantFailClosedSuite(
  wasmPath: string,
): Promise<{ totalFailed: number; tempDirs: string[] }> {
  const validJwt = await makeJwt(3600);

  const missingDir = await mkdtemp(join(tmpdir(), "sso-filter-novariant-"));
  await writeFile(
    join(missingDir, ".env"),
    [
      `FASTEDGE_VAR_SECRET_SESSION_SECRET=${SESSION_SECRET}`,
  ...
  const invalidDir = await mkdtemp(join(tmpdir(), "sso-filter-badvariant-"));
  await writeFile(
    join(invalidDir, ".env"),
    [
      `FASTEDGE_VAR_SECRET_SESSION_SECRET=${SESSION_SECRET}`,
      AUDIENCE_ENV,
      "FASTEDGE_VAR_ENV_SSO_VARIANT=not-a-real-variant",
      "",
    ].join("\n"),
  );
  ...
  const missingSuite = (await runTestSuite(
    defineTestSuite({
      wasmPath,
      runnerConfig: { dotenv: { enabled: true, path: missingDir } },
      tests: [
        {
          name: "SSO_VARIANT unset → 302 (fail-closed) even with a valid token",
          run: async (runner) => {
            const r = await runFlow(runner, {
              url: CDN_URL,
              requestHeaders: { cookie: `${DEFAULT_COOKIE}=${validJwt}` },
            ...
            if (r.finalResponse.status !== 302) {
              throw new Error(
                `expected 302 (filter must refuse sessions with no SSO_VARIANT), got ${r.finalResponse.status}`,
              );
            }
          },
        },
  ...
  report("Suite — fail-closed SSO_VARIANT (missing):", missingSuite);
  totalFailed += missingSuite.failed;

  const invalidSuite = (await runTestSuite(
    defineTestSuite({
      wasmPath,
      runnerConfig: { dotenv: { enabled: true, path: invalidDir } },
      tests: [
        {
          name: "SSO_VARIANT invalid value → 302 (fail-closed)",
          run: async (runner) => {
            const r = await runFlow(runner, {
              url: CDN_URL,
              requestHeaders: { cookie: `${DEFAULT_COOKIE}=${validJwt}` },
            ...
            if (r.finalResponse.status !== 302) {
              throw new Error(
                `expected 302 (filter must refuse sessions with an invalid SSO_VARIANT), got ${r.finalResponse.status}`,
              );
            }
          },
        },
  ...
  report("Suite — fail-closed SSO_VARIANT (invalid):", invalidSuite);
  totalFailed += invalidSuite.failed;

  return { totalFailed, tempDirs: [missingDir, invalidDir] };
}
