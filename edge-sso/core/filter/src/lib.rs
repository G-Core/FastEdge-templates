...
#[cfg(feature = "alg-es256")]
use p256::ecdsa::signature::Verifier as _;

// Exactly one JWT algorithm is pinned per build. The two alg features are
// mutually exclusive, and at least one must be set.
#[cfg(all(feature = "alg-hs256", feature = "alg-es256"))]
compile_error!(
    "alg-hs256 and alg-es256 are mutually exclusive — build one cdn-filter variant at a time"
);
#[cfg(not(any(feature = "alg-hs256", feature = "alg-es256")))]
compile_error!("enable exactly one of the alg-hs256 / alg-es256 features");
...
impl HttpContext for SsoGuard {
    fn on_http_request_headers(&mut self, _: usize, _: bool) -> Action {
        ...
        // never see the raw JWT (baked in at compile time via Cargo feature).
        #[cfg(feature = "strip-session-cookie")]
        {
        ...
        #[cfg(feature = "inject-user-header")]
        {
...
struct JwtClaims {
    #[cfg_attr(not(feature = "inject-user-header"), allow(dead_code))]
    sub: Option<String>,
    iss: Option<String>,
...
fn verify_jwt(token: &str) -> Result<JwtClaims, &'static str> {
    ...
    // The accepted algorithm is pinned at compile time per variant
    // (EXPECTED_ALG), so a token can never select its own verification path.
    // cookie ⇒ ES256-only (the origin holds only a public key); gate-only/header
    // ⇒ HS256-only. The non-pinned branch is not compiled in at all, so even a
    // stray SESSION_SECRET in a cookie deployment's env cannot enable HS256
    // forgery against the asymmetric model.
    if alg != EXPECTED_ALG {
        println!(
            "sso_guard: token alg '{alg}' rejected — this filter accepts only {EXPECTED_ALG}; if the auth-app signs with a different alg, rebuild and redeploy the matching CDN filter variant"
        );
