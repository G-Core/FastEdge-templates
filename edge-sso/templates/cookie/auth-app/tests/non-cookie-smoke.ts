/**
 * Shared smoke suite for the NON-cookie auth-app variants (gate-only, header).
 *
 ...
 * `core/federation/app.tsx`). All federation / token / routing logic is shared
 * `@sso/core`, already covered by the cookie auth-app's unit + integration
 * suites, so this asserts only the variant-specific surface + that the built
 * artifact boots and serves the federation routes.
 */
