...
- **`GCORE_API_TOKEN` has write access to every seed in the store.** Reads via the KV
  REST API never return a seed in the clear — only this app's own `fastedge::kv`
  binding can, at verify time. But the token can overwrite any user's seed via the same
  write endpoint. Use a single-tenant, per-customer isolated KV store and scope
  `GCORE_API_TOKEN` to that one store.
